...
class ShadowRoot(FindElementsMixin):
    ...
    async def find_element_in_shadow(
        ...
        try:
            return await self._find_in_shadow_context(
                safe_selector, method, timeout, raise_exc
            )
        except Exception as e:
            if raise_exc:
                raise ElementNotFound(
                    f"Element '{selector}' not found in shadow root: {e}"
                )
            return None
    ...
    async def find_elements_in_shadow(
        ...
        return await self._find_multiple_in_shadow_context(
            safe_selector, method, timeout
        )
    ...
    def _validate_shadow_root_config(self, object_id: str, mode: str):
        """
    ...
    def _sanitize_selector(self, selector: str, method: str) -> str:
        """
    ...
    async def _find_in_shadow_context(
        ...
        if method == "css":
            ...
            command = DomCommands.query_selector(
                node_id=node_id,
                selector=selector
            )
        elif method == "xpath":
            # For XPath, we need to use performSearch within shadow context
            command = DomCommands.perform_search(
                query=selector,
